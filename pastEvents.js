...
let contenedorPE = document.getElementById("contenedorPE")
let fecha = "2022-12-31"
for (let i = 0; i < events.length; i++) {
    let tarjetaPE = document.createElement("div")
    if (events[i].date < fecha) {
    tarjetaPE.innerHTML = `
        <div class="card2 card m-2 p-1" id="${events[i]._id}">
        <img class="imagenCard" src="./Recursos_Amazing_Events_Task_1/${events[i].image}" class="card-img-top h-50" alt="...">
        <div class="card-body">
        <h5 class="card-title">${events[i].name}</h5>
        <p class="card-text">${events[i].description}</p>
        <div class="d-flex flex-wrap justify-content-between">
        <p class="price fs-5 fw-bold">$${events[i].price}</p>
        <a href="Details.html" class="btn btn-primary">Details</a>
        </div>
        </div>
    `
    }
    contenedorPE.appendChild(tarjetaPE)
}